    ...
    function Dashboard(){
       ...
       $data['sidebar'] = true;
         ...
         if(isset($_SESSION['account_selected'])){
            ...
            $user = $instagram->getUserFeed('self');
            $dados['api'] = $this->object_to_array_recusive($user);            
            $data['content']['rows'][1]['widgets'][1] =  $this->dwoo->get('app/views/dashboard/feed.tpl',$dados);
             
         
        }else{
            $data['content']['rows'][1]['widgets'][1] =  $this->dwoo->get('app/views/first_login_instagram.tpl');
        }
