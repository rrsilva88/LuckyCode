...
/* end template head */ ob_start(); /* template body */ ?><div>
    <div id='form_preview' style="background-color: #EFEFEF;border: 1px solid #CCCCCC;padding: 10px 30px;">
        <h2 <?php if (! (isset($this->scope["preview"]) ? $this->scope["preview"] : null)) {
?>style='display:none;'<?php 
}?>>Preview</h2>
          <?php if ((isset($this->scope["preview"]) ? $this->scope["preview"] : null)) {

echo $this->scope["preview"];
...
<input type='hidden' name='id_evento' value='<?php echo $this->scope["evento"]["id_evento"];?>'>
